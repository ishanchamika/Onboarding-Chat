...
import { Question, HistoryItem, Option, Conversation } from '../Models/conversation.model';

@Injectable({
  providedIn: 'root'
})
export class ConversationService {
  // Sample conversation with flat question structure
...
historyItems.push({
  questionId: current.questionId,    // Add this line
  ...
  timestamp: new Date()              // Add this line
    ...
    if (nextQuestionId && this.conversation.questions[nextQuestionId]) {
      this.currentQuestionSubject.next(this.conversation.questions[nextQuestionId]);
    } else {
      // End of conversation path with no defined next question
      // You could do something special here or just do nothing
    }
  }

  resetConversation(): void {
    this.currentQuestionSubject.next(this.conversation.questions[this.conversation.currentQuestionId]);
